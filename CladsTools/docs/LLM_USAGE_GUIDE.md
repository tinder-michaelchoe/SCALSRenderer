# CladsTools - LLM Usage Guide

**For AI Assistants**: This guide explains how to use CladsTools during development sessions

---

## Overview

CladsTools provides automated code generation and validation utilities designed specifically for LLM-assisted development. These tools help you:
...
**Planned command**:
```bash
swift run clads-test-generator \
  --schema ../Docs/clads-schema.json \
  --component button
```
...
**Planned command**:
```bash
swift run clads-property-validator --component button
```

---

## Development Workflow with CladsTools
...
# Edit: CladsModules/ComponentResolvers/VideoComponentResolver.swift
...
swift run clads-reference-generator --component video
```
...
2. **Run performance profiler** if applicable:
   ```bash
   swift run clads-performance-profiler --example ../CladsExamples/Examples/*.json
   ```

3. **Generate documentation** for new components:
   ```bash
   swift run clads-reference-generator --component newComponent
   ```
...
### Tools
- **Source**: `CladsTools/Sources/[ToolName]/`
- **Executables**: `CladsTools/.build/debug/clads-*`

### Framework Files
- **Components**: `CladsModules/ComponentResolvers/`
- **Tests**: `CLADSTests/Resolution/`
- **Renderers**: `CLADS/Renderers/SwiftUI/`
- **IR**: `CLADS/IR/`
- **Document**: `CLADS/Document/`
...
# Check test patterns
cat ../CLADSTests/Resolution/ComponentResolverTests.swift

# Check renderer patterns
cat ../CLADS/Renderers/SwiftUI/SwiftUINodeRendering.swift
```
