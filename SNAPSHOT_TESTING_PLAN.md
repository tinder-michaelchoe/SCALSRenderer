...
- ✅ Tests are written, run, and verified automatically
- ✅ Tolerance values are adjusted autonomously when needed
- ✅ Renderer and SCALS bugs are fixed as discovered
- ✅ Passing tests are committed automatically
- ✅ Failing tests are documented and skipped after reasonable attempts
- ✅ Progress is tracked in `SNAPSHOT_TEST_PROGRESS.md`
- ❌ **No approval requests** - runs until all test cases are complete
...
ScalsModulesTests/
├── SnapshotTests/
│   ├── Components/              # Individual component snapshots
│   │   ├── TextNodeSnapshotTests.swift
│   │   ├── ButtonNodeSnapshotTests.swift
│   │   ├── ImageNodeSnapshotTests.swift
│   │   └── ...
│   ├── Layouts/                 # Layout component snapshots
│   │   ├── VStackSnapshotTests.swift
│   │   ├── HStackSnapshotTests.swift
│   │   └── ListSnapshotTests.swift
│   ├── CrossRenderer/           # Consistency tests
│   │   ├── ComponentConsistencyTests.swift
│   │   ├── LayoutConsistencyTests.swift
│   │   └── StateBindingConsistencyTests.swift
│   ├── Examples/                # Example-based tests
│   │   ├── ComponentExamplesSnapshotTests.swift
│   │   ├── LayoutExamplesSnapshotTests.swift
│   │   └── ComplexExamplesSnapshotTests.swift
│   ├── Canonical/               # Canonical comparison tests
...
└── __Snapshots__/               # Generated by swift-snapshot-testing
    ├── Components/
    ├── Layouts/
    ├── CrossRenderer/
    ├── Examples/
...
**Pitfall**: 48 examples × 3 renderers × 2 modes = 288+ snapshots
**Solution**:
...
┌─────────────────────────────────────────────────────────────┐
│ 1. Write Test                                                │
│    - Create test method for single component/scenario       │
│    - Choose appropriate snapshot size                       │
...
│    - Run test with record: .all                             │
│    - Inspect snapshot image visually                        │
│    - Verify it looks correct                                │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
│ 3. Run Test (Remove record mode)                            │
│    - Run test normally                                      │
│    - Check if it passes                                     │
└────────────────────┬────────────────────────────────────────┘
                     │
                  ...
                  Passes? ───Yes──────────────────────────┐
                     ...
                     ▼                              ┌──────────────┐
┌─────────────────────────────────────────┐        │ 7. Success!  │
│ 4. Analyze Failure                       │        │   - Commit   │
│    Open test failure details            │        │   - Move on  │
│    Check diff images in test results    │        └──────────────┘
│    Determine root cause:                │
│    a) Tolerance too strict?             │
│    b) Renderer bug?                     │
...
│                                                              │
│    If tolerance issue:                                       │
│    - Increase tolerance slightly (0.01-0.02 increments)     │
│    - Document why in test comment                           │
│    - Go back to Step 3                                      │
│                                                              │
│    If renderer/SCALS bug:                                    │
│    - Fix the bug in renderer or SCALS code                  │
│    - Re-record snapshot if visual output changed           │
│    - Go back to Step 3                                      │
│                                                              │
│    If test setup issue:                                      │
│    - Fix test code (size, node configuration, etc.)         │
│    - Re-record snapshot                                     │
│    - Go back to Step 3                                      │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ▼
          Iteration count > 10? ───No─────┐
                 │                        │
                Yes                       │
                 │                        │
                 ▼                        │
┌─────────────────────────────────────┐  │
│ 6. Document & Revert                │  │
│    - Add to FAILING_TESTS.md        │  │
│    - Document symptoms & attempts   │  │
...
func render(_ node: ButtonNode) -> some View {
    ...
    .padding(node.styles.normal.padding) // ← Added this line
}
...
**Do:**
- ✅ Test one thing at a time (single component, single scenario)
- ✅ Commit after each successful test
- ✅ Document tolerance reasoning in comments
- ✅ Fix renderer bugs as you discover them
- ✅ Revert cleanly after failing tests
- ✅ Keep iteration count reasonable (~10 max)

**Don't:**
- ❌ Write multiple tests before running any
- ❌ Commit failing tests "to fix later"
- ❌ Use extremely high tolerance to force pass (>0.20)
- ❌ Skip visual inspection of snapshots
- ❌ Leave failing test code uncommitted
- ❌ Continue iterating indefinitely on a failing test
...
# Run test → FAIL ❌ (dark mode has 0.015 difference)
...
# Record snapshot
# Run test → FAIL ❌ (font not loading)
...
# Record snapshot
# Run test → FAIL ❌ (attributed string not rendering)
# Investigate SwiftUIRenderer
# Try various fixes → 12 iterations → Still failing
...
- ✅ **DO** make all decisions independently (tolerance values, bug fixes, test reverts)
- ✅ **DO** continue iterating through all planned tests automatically
- ✅ **DO** commit passing tests immediately
- ✅ **DO** document failing tests and move on
- ✅ **DO** fix bugs in renderers/SCALS as discovered
- ❌ **DO NOT** ask for permission before making changes
- ❌ **DO NOT** wait for approval to increase tolerance
- ❌ **DO NOT** stop and ask whether to fix a bug or adjust tolerance
- ❌ **DO NOT** request review before committing
...
- Phase 2: ~30 tests (10 components × 3 tests avg)
...
- ✅ All RenderNode types have snapshot tests
- ✅ SwiftUI, UIKit, and HTML renderers tested
- ✅ Cross-renderer consistency validated
- ✅ All 48 examples tested
- ✅ Canonical comparisons implemented

### Performance Requirements
- ✅ Tier 1 tests complete in <2 minutes
- ✅ Tier 2 tests complete in <5 minutes
- ✅ Full suite completes in <30 minutes
- ✅ Tests can run in parallel

### Quality Requirements
- ✅ <3% difference between SwiftUI/UIKit
- ✅ 1px tolerance for regressions
- ✅ No flaky tests
- ✅ Clear diff images on failure
- ✅ Documented expected differences

### Developer Experience
- ✅ Simple snapshot update workflow
...
- [x] testTextWithBasicStyle (commit: abc123) ✅
- [x] testTextWithColorSchemes (commit: def456) ✅
- [x] testTextWithFontWeights (commit: ghi789) ✅
- [ ] testTextWithAlignment
- [ ] testTextWithLineLimit
- [x] ~~testTextWithAttributedString~~ (See FAILING_TESTS.md) ❌

### TextNode - UIKit
- [x] testTextWithBasicStyle (commit: jkl012) ✅
- [ ] testTextWithColorSchemes
- [ ] testTextWithFontWeights

### ButtonNode - SwiftUI
- [x] testButtonWithBasicStyle (commit: mno345) ✅
...
# 2. Record snapshot
# 3. Run test
# 4. If fail → iterate (adjust tolerance or fix bug)
# 5. If pass → commit and move to next test
# 6. If can't pass after 10 iterations → document in FAILING_TESTS.md and move on
...
```
Test fails?
├─ Tolerance issue? → Increase by 0.01-0.02 → Retry
├─ Renderer bug? → Fix renderer code → Re-record → Retry
├─ SCALS bug? → Fix SCALS code → Re-record → Retry
├─ Test setup issue? → Fix test → Re-record → Retry
└─ Still failing after 10 iterations? → Document in FAILING_TESTS.md → Revert changes → Next test
```
