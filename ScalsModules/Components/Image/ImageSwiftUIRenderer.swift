...
public struct ImageSwiftUIRenderer: SwiftUINodeRendering {
...
struct ImageNodeView: View {
    ...
    private var defaultPlaceholder: ImageSource {
        .sfsymbol(name: "photo")
    }
    
    /// The effective placeholder to show on error or when URL is empty
    private var effectivePlaceholder: ImageSource {
    ...
    @ViewBuilder
    private func renderSource(_ source: ImageSource) -> some View {
        switch source {
        case .sfsymbol(let name):
            Image(systemName: name)
                .resizable()
